...
function buscarprimeirasMedidas(req, res) {
    ...
    console.log(`Recuperando as primeiras ${limite_linhas} medidas`);
    ...
    medidaModel.buscarprimeirasMedidas(idUsuario, limite_linhas).then(function (resultado) {
        ...
        console.log(erro);
        console.log("Houve um erro ao buscar as primeiras medidas.", erro.sqlMessage);
        res.status(500).json(erro.sqlMessage);
    });
}

function buscarMedidas2EmTempoReal(req, res) {

    var idUsuario = req.params.idUsuario;

    console.log(`Recuperando medidas em tempo real`);

    medidaModel.buscarMedidas2EmTempoReal(idUsuario).then(function (resultado) {
        if (resultado.length > 0) {
            res.status(200).json(resultado);
        } else {
            res.status(204).send("Nenhum resultado encontrado!")
